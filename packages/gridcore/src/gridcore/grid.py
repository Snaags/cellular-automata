...
class Board:
    ...
    def replace_contents_at_location(self, location: Location, contents: BaseEntity ):
    ...
    def get_cells_boardering_other(self, team) -> list[Cell]:
            cells_neighbouring_other = []
            for index in range(self.number_of_cells):
                cell = self.cells[index]
                if not cell.is_empty and cell.contents[-1].team == team:
                    possible_neighbours = self.cells[index].location.possible_neighbours()
                    valid_neighbours = self._validate_positions(possible_neighbours)
            ...
            return cells_neighbouring_other
