...
class Board:
    ...
    def get_cells_boardering_empty(self) -> list["Cell"]:
        # Create a boolean array of occupied cells
        occupied = (self._teams != -1)
        
        # Pad with False (meaning empty) so we don't need boundary checks
        padded = np.pad(occupied, pad_width=1, mode='constant', constant_values=False)
        
        # For each occupied cell, check if any neighbor is empty
        ...
        for dy in range(-1, 2):
            for dx in range(-1, 2):
                if dy == 0 and dx == 0:
                    continue
                # Check the slice corresponding to our grid area
    ...
    def get_cells_bordering_empty_for_team(self, team: Union[str, int]) -> list[Location]:
        """Optimized: returns locations of cells that border empty and belong to team."""
        occupied = (self._teams != -1)
    ...
    def get_empty_spawn_locations_for_team(self, team: Union[str, int]) -> list[Location]:
        """Get all valid spawn locations (empty cells adjacent to team's cells)."""
        team_mask = (self._teams == team)
