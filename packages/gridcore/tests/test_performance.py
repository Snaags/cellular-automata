...
class TestSimulationPerformance:
    ...
    def test_advance_tick(self, factions, width, height, ticks, threshold):
        sim = CombatSimulation.initialise_board_with_n_players(
            factions=factions, width=width, height=height, max_ticks=100, rng=random.Random(42)
        )
        start = time.perf_counter()
    ...
    def test_full_simulation_100_ticks(self):
        sim = CombatSimulation.initialise_board_with_n_players(
            factions=5, width=30, height=30, max_ticks=100, rng=random.Random(42)
        )
        start = time.perf_counter()
        for _ in range(100):
            sim.advance({})
        total_elapsed = (time.perf_counter() - start) * 1000
        assert total_elapsed < 1000.0
...
class TestProfilerOutput:
    def test_profile_simulation_prints_stats(self):
        ...
        profiler.enable()
        sim = CombatSimulation.initialise_board_with_n_players(
            factions=3, width=30, height=30, max_ticks=50, rng=random.Random(42)
        )
        for _ in range(50):
            sim.advance({})
        profiler.disable()
