...
class TestProfilerOutput:
    """Tests that verify cProfile can be used to profile the simulation."""

    def test_profile_simulation_prints_stats(self):
        """Profile the simulation and verify stats are collected."""
        ...
        assert stats.total_tt > 0, "Expected positive total time from profiler"
        
        # Print top 5 functions to verify output works
        s.truncate(0)
        s.seek(0)
        stats.print_stats(5)
        output = s.getvalue()
        
        # Verify simulation function appears in profile
        assert 'advance' in output or 'function calls' in output
